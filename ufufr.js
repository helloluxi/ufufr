...
class StorageHandler {
    ...
    async loadData() {
        ...
        await this.getItem('e', text => cube3.updateStats(text));
        await this.getItem('c', text => cube3.updateStats(text));
    ...
    saveStats() {
        const eStats = Object.keys(cube3.stats)
            .filter(key => cube3.stats[key].cat.startsWith('e'))
            .map(key => `${key}=${cube3.stats[key].times},${cube3.stats[key].mark}`)
            .join(';');
        const cStats = Object.keys(cube3.stats)
            .filter(key => cube3.stats[key].cat.startsWith('c'))
            .map(key => `${key}=${cube3.stats[key].times},${cube3.stats[key].mark}`)
            .join(';');
        this.push('e', eStats);
        this.push('c', cStats);
...
function toggleSettingsPanel() {
    ...
    if (panel.style.display === 'flex') {
        ...
        document.getElementById('subset-ckbx').checked = cube3.useSubset > 0;
        ...
        const filtered = Object.keys(cube3.stats).filter(key => cube3.isNonPro(key) && cube3.stats[key].times > 0);
...
function saveSettings(genScr=false) {
    const newSettings = {
        ...
        useSubset: document.getElementById('subset-ckbx').checked ? 4 : 0,
