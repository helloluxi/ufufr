...
function appendSolveHistory(timeMs, scramble) {
    ...
    if (solveHistory.length > HISTORY_LIMIT) solveHistory.pop();
}
...
function formatHistoryEntry(entry) {
    const timeStr = entry.time.toFixed(2);
    const display = entry.dnf ? `DNF(${timeStr})` : timeStr;
    return `${display}  ${entry.scramble}  @${formatHistoryTimestamp(entry.ts)}`;
}
...
document.addEventListener('keydown', function(event) {
    ...
    if (event.key === 'Enter') {
        ...
        if (cmd[0] === '.') {
            ...
            if (solveHistory.length === 0) { print('No solves yet.'); return; }
            solveHistory[0].dnf = !solveHistory[0].dnf;
            print(`Set DNF: ${solveHistory[0].dnf}`);
        } else if (cmd[0] === 'c') {
            ...
            const entries = solveHistory.slice(0, count);
            const fmtTime = e => e.dnf ? `DNF(${e.time.toFixed(2)})` : e.time.toFixed(2);
            let statLabel = '';
            let statValue = '';
            let trimBest = -1, trimWorst = -1;
            if (count === 3 && entries.length === 3) {
                statLabel = 'mo3';
                const hasDnf = entries.some(e => e.dnf);
                statValue = hasDnf ? 'DNF' : (entries.reduce((s, e) => s + e.time, 0) / 3).toFixed(2);
            } else if (count === 5 && entries.length === 5) {
                statLabel = 'ao5';
                const times = entries.map(e => e.dnf ? Infinity : e.time);
                const sorted = [...times].sort((a, b) => a - b);
                trimBest = times.indexOf(sorted[0]);
                trimWorst = times.lastIndexOf(sorted[4]);
                const dnfCount = entries.filter(e => e.dnf).length;
                statValue = dnfCount >= 2 ? 'DNF' : ((sorted[1] + sorted[2] + sorted[3]) / 3).toFixed(2);
            }
            const now = new Date();
            const dateStr = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`;
            const lines = [];
            lines.push(`Generated by BLD Doctor on ${dateStr}`);
            if (statLabel) {
                lines.push(`${statLabel}: ${statValue}`);
            }
            lines.push('');
            lines.push('Time List:');
            entries.forEach((e, i) => {
                let t = fmtTime(e);
                if (i === trimBest || i === trimWorst) t = `(${t})`;
                lines.push(`${i+1}. ${t}   ${e.scramble}`);
            });
            const text = lines.join('\n');
            navigator.clipboard.writeText(text).then(() => {
                const timesList = entries.map(fmtTime).join(', ');
                if (statLabel) {
                    print(`Copied: ${timesList} (${statLabel}: ${statValue})`);
                } else {
                    print(`Copied: ${timesList}`);
                }
            }).catch(err => {
            ...
            print(`? Available commands: .<key>, +<key>, -<key>, m, a, d<mode>, c NUM, t, x, e, i, h, u`);
        }
    }
});
