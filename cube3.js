...
class Cube3 {
    ...
    constructor() {
        ...
        this.useSubset = settings.useSubset !== false;
    ...
    initEmptyStats() {
        ...
        const initStat = (key, cat) => {
            this.stats[key] = { times: 0, mark: 0, sub: Math.random() < 0.04 && this.useSubset, cat: cat };
        };
    ...
    getWeight(key) {
        ...
        if (!stat) return 0;
        return stat.sub ? 2 : stat.times === 0 ? this.learningRate : Math.pow(stat.times, -2 / (1 + stat.mark * this.falloff));
    }
    ...
    getAlgInfo(key) {
        ...
        const stat = this.stats[key] ? `${this.stats[key].mark}/${this.stats[key].times} [${this.getWeight(key).toFixed(2)}]` : '*';
        return [mem, alg, stat];
    }
