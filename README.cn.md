...
按 `KEY=VALUE` 格式填写每个字母对的记忆词或图像提示：
...
- **启动计时器**：长按魔方显示区域或空格键直到变绿
- **停止计时器**：计时过程中轻触魔方显示区域或按任意键即可停止
...
每行格式为 `KEY=VALUE`。空行和不含 `=` 的行会被忽略。

- **alg.txt**：KEY 是你编码方案中的字母对；VALUE 可填写任何想显示的内容（公式、步数、昵称等），会原样存储和显示。
- **mem.txt**：KEY 是字母对；VALUE 是该字母对对应的记忆词或图像提示。

你可以随时用命令行 `a KEY=VAL` 与 `m KEY=VAL` 修改单条内容，无需重新上传整份文件。
